...
<head>
  ...
  <title>Falbor - Authentication</title>
  <style>
...
<head>
  <meta charset="UTF-8">
  <link rel="icon" type="image/svg+xml" href="${FAVICON_DATA_URI}">
  <title>Falbor - MCP Authentication</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    :root {
      --bg: #09090b;
...
if (gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    if (process.platform === "win32") {
      app.setAppUserModelId(IS_DEV ? "dev.21st.Falbor.dev" : "dev.21st.Falbor")
    }

    console.log(`[App] Starting Falbor${IS_DEV ? " (DEV)" : ""}...`)
          ...
          submenu: [
            ...
            { type: "separator" },
            {
              label: isCliInstalled()
                ? "Uninstall 'Falbor' Command..."
                : "Install 'Falbor' Command in PATH...",
                ...
                if (isCliInstalled()) {
                  ...
                  if (result.success) {
                    dialog.showMessageBox({
                      ...
                      detail: "The 'Falbor' command has been removed from your PATH.",
                    })
                    buildMenu()
                  } else {
                  ...
                  if (result.success) {
                    dialog.showMessageBox({
                      ...
                      detail:
                        "You can now use 'Falbor .' in any terminal to open Falbor in that directory.",
                    })
                    buildMenu()
                  } else {
    ...
    // Handle directory argument from CLI (e.g., `Falbor /path/to/project`)
    parseLaunchDirectory()
