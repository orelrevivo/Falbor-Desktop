...
import { chats, getDatabase, projects, subChats, getAll, getOne, runQuery } from "../../db"
import {
...
export const chatsRouter = router({
  ...
  create: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const subChat = await getOne<any>(
        db
          .insert(subChats)
          .values({
            chatId: chat.id,
  ...
  archiveBatch: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const worktreeChats = await getAll(
        db
          .select({ id: chats.id, branch: chats.branch })
          .from(chats)
          .where(inArray(chats.id, input.chatIds))
      )
      const worktreeChatsFiltered = worktreeChats.filter((c) => c.branch != null)
      ...
      if (worktreeChatsFiltered.length > 0) {
        Promise.all(
          worktreeChatsFiltered.map((c) => terminalManager.killByWorkspaceId(c.id)),
        ).then((killResults) => {
          const totalKilled = killResults.reduce((sum, r) => sum + r.killed, 0)
          if (totalKilled > 0) {
            console.log(
              `[chats.archiveBatch] Killed ${totalKilled} terminal session(s) for ${worktreeChatsFiltered.length} worktree workspace(s)`,
            )
          }
        }).catch((error) => {
  ...
  forkSubChat: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const sourceSubChat = await getOne<any>(
        db
          .select()
          .from(subChats)
      ...
      const newSubChat = await getOne<any>(
        db
          .insert(subChats)
          .values({
  ...
  updateSubChat: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      return getOne(
        db.update(subChats).set(updateData).where(eq(subChats.id, input.id)).returning(),
      )
    }),

  /**
   * Update sub-chat message
   */
  updateSubChatMessage: publicProcedure
    .input(z.object({ subChatId: z.string(), message: z.any() }))
    .mutation(async ({ input }) => {
      const db = getDatabase()
      const subChat = await getOne<any>(
        db
      ...
      const msgIndex = messages.findIndex((m: any) => m.id === input.message.id)
      if (msgIndex === -1) throw new Error("Message not found")

      messages[msgIndex] = input.message
  ...
  getDiff: publicProcedure
    ...
    .query(async ({ input }) => {
      const db = getDatabase()
      const chat = await getOne<any>(
        db.select().from(chats).where(eq(chats.id, input.chatId))
      )
  ...
  getPendingPlanApprovals: publicProcedure
    ...
    const allSubChats = await getAll(
      db
        .select({
          chatId: subChats.chatId,
          subChatId: subChats.id,
  ...
  exportChat: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (input.subChatId) {
        ...
        const singleSubChat = await getOne<any>(
          db
            .select()
            .from(subChats)
            .where(and(
