...
import { getDatabase, projects as projectsTable, subChats, getAll, getOne, runQuery } from "../../db"
import {
  fetchMcpTools,
  fetchMcpToolsStdio,
  type McpToolInfo,
...
const DEFAULT_CODEX_MODEL = "gpt-5.2/medium"
...
export async function getAllCodexMcpConfigHandler() {
  ...
  try {
    ...
    const dbProjects = await getAll(db.select({ path: projectsTable.path }).from(projectsTable))
      ...
      return observable<any>((emit) => {
        ...
        ;(async () => {
          try {
            ...
            const existingSubChat = await getOne<any>(
              db
                .select()
                .from(subChats)
            ...
            const persistSubChatMessages = async (messages: any[]) => {
