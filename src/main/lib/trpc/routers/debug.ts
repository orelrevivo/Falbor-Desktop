...
import { getDatabase, projects, chats, subChats, getAll, runQuery } from "../../db"
...
export const debugRouter = router({
  ...
  getDbStats: publicProcedure.query(async () => {
    ...
    const projectResults = await getAll(db.select().from(projects))
    const chatResults = await getAll(db.select().from(chats))
    const subChatResults = await getAll(db.select().from(subChats))

    return {
      projects: projectResults.length,
      chats: chatResults.length,
      subChats: subChatResults.length,
    }
  }),
  ...
  clearChats: publicProcedure.mutation(async () => {
    ...
    await runQuery(db.delete(subChats))
    await runQuery(db.delete(chats))
  ...
  clearAllData: publicProcedure.mutation(async () => {
    ...
    await runQuery(db.delete(subChats))
    await runQuery(db.delete(chats))
    await runQuery(db.delete(projects))
