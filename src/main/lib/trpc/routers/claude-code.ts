...
function storeOAuthToken(oauthToken: string, setAsActive = true): string {
  ...
  runQuery(db.delete(claudeCodeCredentials).where(eq(claudeCodeCredentials.id, "default")))
...
export const claudeCodeRouter = router({
  ...
  disconnect: publicProcedure.mutation(async () => {
    ...
    if (settings?.activeAccountId) {
      ...
      const firstRemaining = await getOne<any>(db.select().from(anthropicAccounts).limit(1))
