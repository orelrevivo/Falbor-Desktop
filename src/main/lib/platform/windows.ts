...
export class WindowsPlatformProvider extends BasePlatformProvider {
  ...
  getCliConfig(): CliConfig {
    ...
    return {
      installPath: path.join(home, ".local", "bin", "Falbor.cmd"),
      scriptName: "Falbor.cmd",
  ...
  async installCli(
    ...
    try {
      ...
      console.log("[CLI] Installed Falbor command to", installPath)
      console.log(
      ...
      return {
        success: true,
        pathHint: `To use Falbor from terminal, add to your PATH: ${installDir}`,
      }
    } catch (error: unknown) {
      const errorMessage =
  ...
  async uninstallCli(): Promise<{ success: boolean; error?: string }> {
    ...
    try {
      ...
      console.log("[CLI] Uninstalled Falbor command")
      return { success: true }
    } catch (error: unknown) {
      const errorMessage =
