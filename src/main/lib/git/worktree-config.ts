...
export type WorktreeConfigSource = "custom" | "cursor" | "Falbor" | null
...
const ONECODE_CONFIG_PATH = ".Falbor/worktree.json"
 ...
 * Priority: custom path > .cursor/worktrees.json > .Falbor/worktree.json
 */
export async function detectWorktreeConfig(
  ...
  if (await fileExists(onecodePath)) {
    ...
    if (config) {
      return { config, path: onecodePath, source: "Falbor" }
    }
  }
...
export async function executeWorktreeSetup(
  ...
  for (const cmd of commandList) {
    ...
    try {
      ...
      result.commandsRun++
      console.log(`[worktree-setup] âœ“ ${cmd}`)
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : String(error)
      result.errors.push(errorMsg)
      result.output.push(`[error] ${errorMsg}`)
      console.error(`[worktree-setup] âœ— ${cmd}: ${errorMsg}`)
