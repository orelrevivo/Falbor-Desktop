...
function getSelectedCodexModel(subChatId: string): string {
  ...
  // If the model has no thinking levels defined, just return the model ID
  if (!selectedModel.thinkings || selectedModel.thinkings.length === 0) {
  ...
  const isStandardOpenAIModel =
    selectedModel.id.startsWith("gpt-") ||
    selectedModel.id.startsWith("o1") ||
    selectedModel.id.startsWith("o3")
