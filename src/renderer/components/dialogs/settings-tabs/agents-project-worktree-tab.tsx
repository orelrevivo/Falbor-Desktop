...
function ProjectDetail({ projectId }: { projectId: string }) {
  ...
  // Save mutation (auto-save, no toast on success â€” only on error)
  ...
  // Local state
  const [saveTarget, setSaveTarget] = useState<"cursor" | "Falbor">("Falbor")
  ...
  useEffect(() => {
    if (configData) {
      const newSaveTarget = configData.source === "cursor" ? "cursor" : "Falbor"
      ...
      <div className="max-w-2xl mx-auto p-6 space-y-6">
          ...
          <div className="bg-background rounded-lg border border-border overflow-hidden">
            ...
            <div className="flex items-center justify-between p-4 border-t border-border">
              <div className="flex-1 min-w-0 mr-4">
                ...
                <p className="text-sm text-muted-foreground truncate">{project?.path || "â€”"}</p>
              </div>
              <Button
                variant="outline"
                size="sm"
          ...
          <div className="bg-background rounded-lg border border-border overflow-hidden">
            <div className="flex items-center justify-between p-4">
              ...
              <Select
                value={saveTarget}
                onValueChange={(v) => {
                  setSaveTarget(v as "cursor" | "Falbor")
                  pendingSaveRef.current = true
                }}
              >
                <SelectTrigger className="w-auto px-3">
                  <span className="text-sm font-mono">
                    {saveTarget === "cursor" ? ".cursor/worktrees.json" : ".Falbor/worktree.json"}
                  </span>
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="Falbor">.Falbor/worktree.json</SelectItem>
                  {cursorExists && (
        ...
        <div>
          ...
          <div className="bg-background rounded-lg border border-border overflow-hidden">
            ...
            {/* Platform overrides â€” macOS/Linux */}
