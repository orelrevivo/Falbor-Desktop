// Patches the Electron.app bundle in node_modules to show "Falbor" name and icon in macOS dock during dev mode.
...
if (existsSync(plistPath)) {
  try {
    execSync(`/usr/libexec/PlistBuddy -c "Set :CFBundleName Falbor" "${plistPath}"`)
    execSync(`/usr/libexec/PlistBuddy -c "Set :CFBundleDisplayName Falbor" "${plistPath}"`)
    console.log("[patch-electron-dev] Updated Info.plist: name -> Falbor")
  } catch (e) {
