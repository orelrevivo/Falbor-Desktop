...
# Sync desktop app to public Falbor repository
...
PUBLIC_REPO="git@github.com:21st-dev/Falbor.git"
PUBLIC_REPO_HTTPS="https://github.com/21st-dev/Falbor"
PRIVATE_REPO="21st-dev/21st"
TEMP_DIR="/tmp/Falbor-sync-$$"

echo "ðŸ”„ Syncing desktop app to public repository..."
echo "   Version: $VERSION"
echo "   Tag: $TAG"
echo ""

# Get release notes from private repo
echo "ðŸ“ Fetching release notes from private repo..."
RELEASE_NOTES=$(gh release view "$TAG" --repo "$PRIVATE_REPO" --json body -q '.body' 2>/dev/null || echo "")

if [ -z "$RELEASE_NOTES" ]; then
    echo "âš ï¸  No release found for $TAG in private repo"
    echo "   Please create a release in the private repo first:"
    echo "   gh release create $TAG --title \"Falbor $TAG\" --notes \"...\""
    echo ""
...
if git diff --staged --quiet; then
    echo "â„¹ï¸  No code changes to sync"
else
...
echo ""
echo "ðŸ·ï¸  Creating GitHub release in public repo..."
...
echo "   ðŸ”— $PUBLIC_REPO_HTTPS/releases/tag/$TAG"
...
echo ""
echo "âœ¨ Done! Public repo synced with release $TAG"
